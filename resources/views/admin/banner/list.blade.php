    ...
    <div class="card">
        <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
            ...
            <div class="form-search form-inline">
                <form action="{{route('admin.banner.list')}}" method="GET" class="d-flex">
        ...
        <div class="card-body">
            ...
            <form action="{{route('admin.banner.action')}}" method="POST">
                @csrf
                ...
                <table class="table table-striped table-checkall">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <th scope="row">{{++$i}}</th>
                            <td><img src="{{asset($banner->thumb_banner)}}" class="imgg" data-id="{{ $banner->id }} "
                                    data-toggle="modal" data-target="#exampleModalCenter" alt="" width="80px"
                                    height="40">
                            </td>
                            <td><a href="" class="link" data-id="{{ $banner->id }}" data-toggle="modal"
                                    data-target="#exampleModalCenter">{{Str::limit($banner->link,
                                    $limit = 20, $end =
                                    '...')}}</a></td>
                            <td>{{$banner->sort}}</td>
                            <td class="text-center">
                                @if($banner->users)
                                    <small><b>{{$banner->users->name}}</b></small>
                                    <br>
                                    @if(!empty($banner->users->roles) && isset($banner->users->roles[0]))
                                        <p class="bg-secondary text-white rounded px-1 text-center d-inline-block mb-0">
                                            {{$banner->users->roles[0]['name']}}</p>
                                    @else
                                        <p class="bg-secondary text-white rounded px-1 text-center d-inline-block mb-0">
                                            Chưa phân vai trò</p>
                                    @endif
                            ...
                            <td>{!!($banner->status == 0)?'<span class="badge badge-warning">Chờ duyệt</span>':'<span
                                    class="badge badge-success">Công khai</span>'!!}</td>
                            <td>
                                <a data-toggle="modal" data-id="{{ $banner->id }}" data-target="#exampleModalCenter"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Thông tin banner</h5>
